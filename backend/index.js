...
async function searchVectorDatabase(query, matchCount = 5, threshold = 0.50) {
...
4. **คัดเลือกเฉพาะ "เอกสารข้อมูล" ที่เนื้อหาตรงกับกลุ่มเป้าหมายในคำถามเท่านั้น (เช่น ถามเรื่องผู้กู้รายเก่า ให้ใช้เฉพาะเนื้อหาของผู้กู้รายเก่าแล้วเมินเอกสารอื่นทิ้งไปเลย) จากนั้นนำมาสังเคราะห์เป็นคำตอบให้เข้าใจง่าย**
5. **(สำคัญมาก!) หากผู้ใช้ถามหาแหล่งที่มา ลิงก์ หรืออ้างอิง และในเอกสารมีลิงก์ (URL) หรือข้อความ "อ้างอิงจาก" ปรากฏอยู่ คุณต้องคัดลอกลิงก์นั้นส่งให้ผู้ใช้อย่างชัดเจน**
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        if (botResponse === null) {
            ...
            let retrievedDocs = await searchVectorDatabase(search_topic, 5, 0.50);
