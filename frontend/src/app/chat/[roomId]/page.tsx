...
export default function ChatRoomPage() { // ‼️ V33 CHANGE: เปลี่ยนชื่อ Component
  ...
  useEffect(() => {
    ...
    if (!loading && user && roomId) {
      ...
      fetch(`http://localhost:3001/api/chat/room/${roomId}`) // (ใช้ http://localhost:3001 ถ้า dev)
        .then((res) => res.json())
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/chat', { // (ใช้ http://localhost:3001 ถ้า dev)
        method: 'POST',
